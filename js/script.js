...
function CalcRoute(pEnd, pName, pRef, pCt) {
	...
	serviceDirection.route(request, function(result, status) {
		if (status == google.maps.DirectionsStatus.OK) {
			...
			$('#'+pRef+' .locations').html('<a href="http://maps.google.com/?saddr='+searchRequest.location.lat()+","+searchRequest.location.lng()+'&daddr='+result.routes[0].legs[0].end_address+'" class="btn btn-large" target="_blank""><i class="icon-road"></i>&nbsp;'+result.routes[0].legs[0].distance.text+'</a>');
		}
		StartGetDistance();
	});
}
