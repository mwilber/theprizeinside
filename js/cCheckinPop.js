...
CheckinPop.prototype.HandleCheckinData = function(self){
    return function(response) {
		...
		if( response.profile.profileNickname == undefined ){
		  self.panel.elem.find('.nickname').html('Anonymous');
		}else{
		  self.panel.elem.find('.nickname').html(response.profile.profileNickname); 
		}
		if( response.profile.profilePicture == undefined ){
		    self.panel.elem.find('.avatar').attr('src','/img/anonymous-user.png');
		}else{
