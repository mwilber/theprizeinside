...
class Review extends CI_Controller
{
	...
	function index($pFormat="html")
	{
		...
		$data['total_rows'] = $this->$model_ref->GetWithRestaurants(array('count' => true));
		$data['records'] = $this->$model_ref->GetWithRestaurants(array('prizeActive'=>1,'sortBy'=>$this->$model_ref->_table().'.restaurantId','sortDirection'=>'ASC'));
		$data['fields'] = $this->$model_ref->_fields();
	...
	function deactivate($recordId)
	{
	    ...
	    if($this->$model_ref->Update(array('prizeId'=>$recordId,'prizeActive'=>0)))
        {
            $this->session->set_flashdata('flashConfirm', 'The user has been successfully updated.');
            redirect($this->uri->segment(1));
        }
	    ...
	    $this->session->set_flashdata('flashConfirm', 'The record has been successfully deactivated.');
	    redirect($this->uri->segment(1));
	}
}